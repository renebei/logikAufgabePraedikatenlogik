...
public class PraenexnormalformTest {
  ...
  @Test
  public void testPraenexnormalformSchrittZwei1BeiClone() {
    Variable vv = new Variable(vi.getName(), VarTyp.INT);
    Exists fu1 = new Exists(vv, r1);
    Variable vw = new Variable(vi.getName(), VarTyp.INT);
    ForAll fu2 = new ForAll(vw, r2.deepClone());
    Formel fu3 = new Und(fu1, fu2);
    //System.out.println(fu3.gebunden());
    Formel vorher = fu3.deepClone();
    fu3 = fu3.praenexnormalformSchritt2();
    Variable vvneu = ((Exists) fu3.getOperanden().get(0)).getVar();
    Variable vwneu = ((ForAll) fu3.getOperanden().get(1)).getVar();
    Assertions.assertNotEquals(vvneu, vv);
...
//    Assertions.assertSame(f1.getTeilterme().get(0), vvneu);
//    Assertions.assertSame(f1.getTeilterme().get(1), vvneu);
    Relation r1neu = (Relation) (fu3.getOperanden().get(0).getOperanden().get(0));
    Assertions.assertSame(
            r1neu.getTerme().get(0).getTeilterme().get(0), vvneu, "Variable " + vvneu.zeigen()
                    + " muss in " + r1neu.getTerme().get(0).zeigen()
                    +"  an erster Stelle in " + fu3.zeigen() + " stehen, vorher: " + vorher.zeigen());
    Assertions.assertSame(
            r1neu.getTerme().get(0).getTeilterme().get(1), vvneu, "Variable " + vvneu.zeigen()
                    + " muss in " + r1neu.getTerme().get(0).zeigen()
                    +"  an zweiter Stelle in " + fu3.zeigen() + " stehen, vorher: " + vorher.zeigen());

    Funktion r2f1 = (Funktion)(((Relation) (fu3.getOperanden()
            .get(1).getOperanden().get(0))).getTerme().get(0));
    Funktion r1f1 = (Funktion)(r1neu.getTerme().get(0));
    Assertions.assertEquals(r1f1.getName(), r2f1.getName());
    Assertions.assertSame(vvneu, r1f1.getTeilterme().get(0));
    Assertions.assertSame(vvneu, r1f1.getTeilterme().get(1));
    Assertions.assertSame(vwneu, r2f1.getTeilterme().get(0), "Variable " + vwneu.zeigen()
            + " muss in " + r2f1.zeigen()
            +"  an zweiter Stelle in " + fu3.zeigen() + " stehen, vorher: " + vorher.zeigen());
    Assertions.assertSame(vwneu, r2f1.getTeilterme().get(1));
  }

  @Test
  public void testPraenexnormalformSchrittZwei2BeiClone() {
    Variable vv = new Variable(vi.getName(), VarTyp.INT);
    Exists fu1 = new Exists(vv, r1);
    // Variable vw = new Variable(vi.getName(), VarTyp.INT);
    ForAll fu2 = new ForAll(vv, r2.deepClone());
    Formel fu3 = new Und(fu1, fu2);
    System.out.println(fu3.gebunden());
    System.out.println(fu3.frei());
    System.out.println(fu3.zeigen());
    fu3 = fu3.praenexnormalformSchritt2();
    Variable vvneu = ((Exists) fu3.getOperanden().get(0)).getVar();
    Variable vwneu = ((ForAll) fu3.getOperanden().get(1)).getVar();
    Assertions.assertNotEquals(vvneu, vv);
    Assertions.assertNotEquals(vwneu, vv);
    Assertions.assertNotEquals(vvneu, vwneu, "es muessen unterschiedliche Variablen unter den"
            + " Quantoren stehen in " + fu3.zeigen());
//    Assertions.assertSame(f1.getTeilterme().get(0), vvneu);
//    Assertions.assertSame(f1.getTeilterme().get(1), vvneu);
    Relation r2neu = (Relation) (fu3.getOperanden().get(1).getOperanden().get(0));
    Assertions.assertSame(
            r2neu.getTerme().get(0).getTeilterme().get(0), vwneu);
    Assertions.assertSame(
            r2neu.getTerme().get(0).getTeilterme().get(1), vwneu);
    Funktion r1f1 = (Funktion)(((Relation) (fu3.getOperanden()
            .get(0).getOperanden().get(0))).getTerme().get(0));
    Funktion r2f1 = (Funktion)(r2neu.getTerme().get(0));
    Assertions.assertEquals(r1f1.getName(), r2f1.getName());
    Assertions.assertSame(vvneu, r1f1.getTeilterme().get(0));
    Assertions.assertSame(vvneu, r1f1.getTeilterme().get(1));
    Assertions.assertSame(vwneu, r2f1.getTeilterme().get(0));
    Assertions.assertSame(vwneu, r2f1.getTeilterme().get(1));
  }
